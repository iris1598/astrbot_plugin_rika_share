...
from ..models import ImageContent, Platform
from ..models.platforms.weibo import (
    article_decoder,
    status_decoder,
    video_show_decoder,
)
...
class WeiBoParser(BaseParser):
    ...
    async def parse_fid(self, fid: str):
        ...
        data = video_show_decoder.decode(response.content).data
    ...
    async def parse_weibo_id(self, weibo_id: str):
        ...
        weibo_data = status_decoder.decode(response.content).data
...
ADAPTER = register_adapter(
    AdapterSpec(
        name=PlatformEnum.WEIBO.value,
        url_pattern=re.compile(
            r"(weibo\.com|weibo\.cn|m\.weibo\.cn|video\.weibo\.com|mapp\.api\.weibo\.cn)"
        ),
