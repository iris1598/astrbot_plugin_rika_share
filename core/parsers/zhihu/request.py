...
from ...base_parser import ParseException, COMMON_TIMEOUT
...
class ZhihuRequestMixin:
    async def _fetch_initial_data(
        ...
        *,
        ...
        cookie = getattr(get_config(), "ZHIHU_CK", None)
        if saw_challenge:
            if cookie:
    ...
    def _build_request_headers(
        ...
        *,
        ...
        headers.update(
            {
                "accept": accept or self.headers.get("accept", "*/*"),
                "accept-language": self.headers.get("accept-language", "zh-CN,zh;q=0.9"),
                "referer": self.headers.get("referer", "https://www.zhihu.com/"),
                "origin": self.headers.get("origin", "https://www.zhihu.com"),
                "cache-control": self.headers.get("cache-control", "no-cache"),
                "pragma": self.headers.get("pragma", "no-cache"),
            }
        )
        cookie = getattr(get_config(), "ZHIHU_CK", None)
        if cookie:
        ...
        *,
        ...
        def do_request():
            return curl_requests.get(
                ...
                timeout=COMMON_TIMEOUT.connect + COMMON_TIMEOUT.read if hasattr(COMMON_TIMEOUT, "connect") else 20.0,
