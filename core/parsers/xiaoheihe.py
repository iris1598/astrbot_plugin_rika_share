...
from ..base_parser import BaseParser, handle, PlatformEnum, ParseException, COMMON_TIMEOUT
...
class XiaoheiheParser(BaseParser):
    platform: ClassVar[Platform] = Platform(name=PlatformEnum.XIAOHEIHE, display_name="小黑盒")
    ...
    def _build_video_content_from_url(
        ...
        if path.endswith(".m3u8"):
            task = self.downloader.download_m3u8(
                video_url, ext_headers=self.headers
            )
